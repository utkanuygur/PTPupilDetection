...
def predict_mask(model, input_image, device):
    ...
    return prediction.cpu().numpy().squeeze(0).squeeze(0)

def apply_mask_to_image(image, mask):
    mask = Image.fromarray((mask * 255).astype(np.uint8)).convert("RGBA")
    image = image.convert("RGBA")
    masked_image = Image.blend(image, mask, alpha=0.5)
...
def process_images(model, device, data_loader, image_height, image_width, output_dir):
    for input_image, image_path in data_loader:
        input_image = input_image.to(device)
        predicted_mask = predict_mask(model, input_image, device)
        original_image = Image.open(image_path[0]).convert("L").resize((image_width, image_height))
        masked_image = apply_mask_to_image(original_image, predicted_mask)
        save_path = os.path.join(output_dir, os.path.splitext(os.path.basename(image_path[0]))[0] + ".jpg")
